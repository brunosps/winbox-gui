...
pub fn render_office_install_script() -> String {
    crlf(&[
        ...
        "$odtDir = Join-Path $shareRoot 'odt'",
        "$markerDir = Join-Path $shareRoot 'markers'",
        "$logDir = Join-Path $shareRoot 'logs'",
        "New-Item -ItemType Directory -Force -Path $markerDir, $logDir | Out-Null",
        "$setupPath = Join-Path $odtDir 'setup.exe'",
        "$configPath = Join-Path $odtDir 'configuration.xml'",
        "$markerPath = Join-Path $markerDir 'office_install.json'",
        "$logPath = Join-Path $logDir 'office_install.log'",
        "function Write-WinboxMarker($status, $exitCode, $errorCode, $message) {",
        "  $payload = [ordered]@{ phase = 'office_install'; status = $status; exitCode = $exitCode; updatedAt = (Get-Date).ToUniversalTime().ToString('o') }",
        "  if ($errorCode) { $payload.error = [ordered]@{ code = $errorCode; message = $message; logPath = 'logs/office_install.log' } }",
        "  $payload | ConvertTo-Json -Compress | Set-Content -Path $markerPath -Encoding UTF8",
        "}",
        "try {",
        "  Write-WinboxMarker 'running' $null $null $null",
        "  Start-Transcript -Path $logPath -Append -ErrorAction SilentlyContinue | Out-Null",
        "  if (-not (Test-Path $setupPath)) { throw 'setup.exe ausente no share ODT' }",
        "  if (-not (Test-Path $configPath)) { throw 'configuration.xml ausente no share ODT' }",
        "  & $setupPath /configure $configPath",
        "  $exitCode = $LASTEXITCODE",
        "  Stop-Transcript -ErrorAction SilentlyContinue | Out-Null",
        "  if ($exitCode -ne 0) { Write-WinboxMarker 'failed' $exitCode 'office_odt_failed' \"ODT retornou $exitCode\"; exit $exitCode }",
        "  Write-WinboxMarker 'done' 0 $null $null",
        "  exit 0",
        "} catch {",
        "  $message = $_.Exception.Message",
        "  Write-WinboxMarker 'failed' 1 'office_odt_failed' $message",
        "  Stop-Transcript -ErrorAction SilentlyContinue | Out-Null",
        "  exit 1",
        "}",
    ])
